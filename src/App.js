...
const tools = [{
  functionDeclarations: [
    ...
    },
    {
      ...
      parameters: {
        type: "OBJECT",
        properties: { expression: { type: "STRING", description: "Math expression e.g. '250 * 0.18'" } },
        required: ["expression"]
      }
    },
    {
...
async function search_web({ query }) {
  try {
    ...
    if (!answer) return { result: `No live result found. Please answer "${query}" using your own knowledge.` };
...
function calculate({ expression }) {
  ...
  if (banned.some(w => expression.includes(w))) return { result: "Invalid expression." };
  try {
...
async function github_pr({ action, title, head, base, body, pr_number }) {
  ...
  try {
    if (action === "create") {
      const res = await fetch(`${BASE_URL}/pulls`, { method: "POST", headers, body: JSON.stringify({ title, head, base, body: body || "PR created by Gossaye AI 🤖" }) });
      const data = await res.json();
    ...
    if (action === "review") {
      ...
      return { result: `🔍 PR #${pr_number}: ${pr.title}\n👤 ${pr.user.login}\n🔀 ${pr.head.ref} → ${pr.base.ref}\n📁 ${files.length} files\n🔗 ${pr.html_url}` };
    }
  } catch (err) {
...
const WELCOME = "👋 Hey! I'm your AI Agent with 3 tools:\n🧮 Calculator  🔍 Web Search  🐙 GitHub PR\n\nWhat can I help you with?";
...
3. github_pr — GitHub Pull Request actions
...
export default function App() {
  ...
  }, [messages, toolStatus, streamingText]);
  ...
  async function sendMessage() {
    ...
    try {
      const orchestratorAnswer = await runOrchestrator(userText, (status) => setToolStatus(status));
      ...
      if (orchestratorAnswer) {
        ...
        await callBackendStream(
          userText, 'react-ui',
          (chunk) => { fullAnswer += chunk; setStreamingText(fullAnswer); },
          (toolName) => setToolStatus(TOOL_LABELS[toolName] || '🤖 Processing...'),
          (data) => {
        ...
        const answer = await runAgent(userText, history, (toolName) => setToolStatus(TOOL_LABELS[toolName] || '🤖 Thinking...'), userProfile);
        setMessages(prev => {
    ...
    <div className="app">
      ...
      <div className="header">
        ...
        <div className="header-info">
          ...
          <div className="tagline">
            <span className={`status ${loading ? 'busy' : 'online'}`}>
      ...
      {(userProfile.name || userProfile.job || userProfile.location) && (
        <div className="profile-bar">
          <span className="profile-label">🧠 Known</span>
          ...
          <button className="profile-clear" onClick={() => { clearProfile(); setUserProfile(loadProfile()); }}>✕</button>
        </div>
      )}
      ...
      {lastCost && (
        <div className="cost-bar">
          <span style={{fontSize:10,color:'#334155',fontWeight:600,textTransform:'uppercase',letterSpacing:'0.5px'}}>Last request</span>
      ...
      <div className="messages">
        ...
        {toolStatus && (
          <div className="message bot">
            <div className="avatar">🤖</div>
            <div className="bubble tool-status">
        ...
        {/* Thinking dots */}
        {loading && !toolStatus && !isStreaming && (
          <div className="message bot">
            <div className="avatar">🤖</div>
      ...
      <div className="tool-pills">
        ...
        <span className="tool-pill">🐙 GitHub PR</span>
      </div>
      ...
      <div className="input-area">
        ...
        <button className="send-btn" onClick={sendMessage} disabled={loading || !input.trim()}>
          {loading ? <><div className="tool-spinner" style={{borderTopColor:'white',width:12,height:12}}/> </> : '↑ Send'}
        </button>
      </div>
