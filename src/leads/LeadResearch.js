...
export default function LeadResearch() {
  ...
  function fetchFavicon(companyName) {
    setFavicon(`https://www.google.com/s2/favicons?domain=${companyName.toLowerCase()}.com&sz=64`);
  }
  ...
  function exportCSV() {
    ...
    const rows = history.map(h => [
      h.company, parseScore(h.score), parseTier(h.score),
      parseBudget(h.score), new Date(h.timestamp).toLocaleString()
    ]);
  ...
  return (
    <div className="lrp-root">
                    ...
                    return (
                      <div key={step.id} className={`lrp-agent-step ${st || 'idle'}`}>
                        <div className="lrp-step-icon">
                          {st === 'done' ? '✅' : st === 'running'
                            ? <span className="lrp-mini-spinner"/> : step.icon}
                        </div>
                        <div className="lrp-step-info">
                          <div className="lrp-step-label">{step.label}</div>
                          {st === 'running' && <div className="lrp-step-desc">{step.desc}</div>}
                        </div>
      ...
      {/* ── PAGE TABS ────────────────────────────────────────────────────── */}
      <div style={{
        maxWidth:1200, margin:'0 auto', padding:'16px 40px 0',
        display:'flex', gap:8, borderBottom:'1px solid #1e293b'
      }}>
        {[
          ...
          <button key={t.id} onClick={() => setPageTab(t.id)} style={{
            ...
            border:'1.5px solid', marginBottom:-1,
            borderColor:       pageTab===t.id ? '#1e293b'  : 'transparent',
            borderBottomColor: pageTab===t.id ? '#060a14'  : 'transparent',
            background:        pageTab===t.id ? '#060a14'  : 'transparent',
            color:             pageTab===t.id ? '#f1f5f9'  : '#475569',
                ...
                <div className="lrp-score-hero" style={{
                  ...
                  <div className="lrp-score-left">
                    ...
                    <div>
                      ...
                      <div className="lrp-tier-pill" style={{ background: tierConf.color }}>
                        {tierConf.label}
                      </div>
          ...
          <div className="lrp-sidebar">
            <div className="lrp-sidebar-header">
              ...
              <div className="lrp-sidebar-actions">
                {history.length > 0 && (
                  <>
                    <button className="lrp-icon-btn" onClick={exportCSV} title="Export CSV">📥</button>
                    <button className="lrp-icon-btn danger" onClick={clearHistory} title="Clear history">🗑️</button>
                  </>
                )}
              </div>
            </div>
                  ...
                  return (
                    <div key={i} className="lrp-history-item"
                      ...
                      <div className="lrp-hi-bottom">
                        <span className="lrp-hi-tier" style={{ background: tc.bg, color: tc.color }}>
                          {tc.label}
                        </span>
